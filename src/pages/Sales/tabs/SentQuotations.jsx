...
export default function SentQuotations() {
  ...
  const stats = useMemo(() => {
    ...
    const inNegotiation = quotations.filter((q) =>
      ["negotiation_requested", "counter_offered"].includes(q.status)
    ).length;
    const pendingResponse = quotations.filter((q) =>
      ["sent", "viewed"].includes(q.status)
    ).length;
    const closedOrRejected = quotations.filter((q) =>
      ["closed", "rejected"].includes(q.status)
    ).length;
    return { total, accepted, inNegotiation, pendingResponse, closedOrRejected };
  }, [quotations]);
